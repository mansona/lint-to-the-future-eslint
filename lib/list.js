 ...
 */
async function getSvelteFileRules(file, filePath) {
  const ast = await parseSvelteFile(file, filePath);
...
export default async function list(cwd = process.cwd()) {
  ...
  await Promise.all(
    files.map(async (relativeFilePath) => {
      ...
      if (extname(filePath) === '.svelte') {
        rules = await getSvelteFileRules(file, filePath);
      } else {
