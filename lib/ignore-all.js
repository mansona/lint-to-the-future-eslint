...
const ESLINT_DISABLE_REGEX = /eslint-disable (.*)\*\//;
 ...
 * @returns {{ hasExisting: boolean, uniqueIds: string[] }}
 */
...
function addDisableToScript(file, script, ruleIds) {
  // Find the end of the opening script tag (after the `>`)
  ...
  // Find the first non-empty line to check for existing disable
  ...
  const firstLine = contentLines[firstContentLineIndex];
...
function scriptHasDisable(file, script) {
  ...
  for (const line of contentLines) {
    ...
    return ESLINT_DISABLE_REGEX.test(trimmed);
  }
  return false;
}
...
async function ignoreError(error) {
  ...
  if (extname(error.filePath) === '.svelte') {
    try {
      // Dynamic import to avoid requiring svelte in non-Svelte projects
      const { parse } = await import('svelte/compiler');
      const ast = parse(file, {
        filename: error.filePath,
        modern: true,
        // in case there's some syntax error we want to try to recover anyway
        loose: true,
      });
      ...
      /** In case parsing fails for some reason we default to the normal behavior */
    }
  }
